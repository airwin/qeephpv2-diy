...
abstract class QDebug
{
    ...
    static function dump($vars, $label = null, $depth = null)
    {
        ...
        if (val($last, 'class') == 'QDebug' && val($last, 'function') == 'dump')
        {
            $last = array_shift($trace);
        }
        ...
        if ($html_output === 'auto')
        {
            $html_output = ini_get('htmlOutput') ?: ini_get('html_errors');
        }
        $dump = new QDebug_Dump($html_output, $depth);
