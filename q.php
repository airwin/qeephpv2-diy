 ...
 * 框架基本库所在路径
 */
define('Q_PATH', rtrim(__DIR__, '/\\') . DS);
...
class Q
{
     ...
     */
    static function singleton($class_name, $attr = null)
    {
        $key = strtolower($class_name) . md5(serialize($attr));
        if (isset(self::$_objects[$key]))
        {
            return self::$_objects[$key];
        }
        self::loadClass($class_name);
        return self::register(new $class_name($attr), $class_name);
    }
    ...
    static function cache($id, array $policy = null, $backend_class = null)
    {
        ...
        if (is_null($backend_class))
        {
            if (is_null($obj))
            {
                $obj = self::singleton(self::ini('runtime_cache_backend'), $policy );
            }
            return $obj->get($id, $policy);
        }
        else
        {
            $cache = self::singleton($backend_class, $policy);
 ...
 * @return string
 */
function dump($vars, $label = null, $return = false, $depth = null)
{
    if ($return) ob_start();
    QDebug::dump($vars, $label, $depth);
