...
class QColl implements Iterator, ArrayAccess, Countable
{
    ...
    {
        if(!$prop_name) return $this->_coll;
        $return = array();
     ...
     * @return array 
     */
    function slice($offset = 0, $length = null, $preserve_keys = false)
    {
        return array_slice($this->_coll, $offset, $length, $preserve_keys);
    }
     ...
     * @return mixed
     */
    function shift($preserve_keys = false)
    {
        $ret = reset($this->_coll);
        $this->_coll = array_slice($this->_coll, 1, $this->count() - 1, $preserve_keys);
        return $ret;
    }

    /**
     * Unshift an element into the beginning of QColl
     *
     * @return mixed
     */
    function unshift($data)
    {
        array_unshift($this->_coll, $data);
     ...
     */
    function append($data, $keep_keys = false)
    {
        if (is_array($data) || ($data instanceof Iterator))
        {
            foreach ($data as $offset => $item)
            {
                $this->offsetSet($keep_keys ? $offset : null, $item);
            }
        }
        else
        {
     ...
     * $coll = QColl::createFromArray(array(
     *     new OrderItem(100, 3),
     *     new OrderItem(200, 5),
