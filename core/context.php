...
class QContext implements ArrayAccess
{
     ...
     * UDI 参数的默认值
     */
	private static $_udi_param = array(
        self::UDI_MODULE => self::UDI_MODULE,
        self::UDI_NAMESPACE => self::UDI_NAMESPACE,
        self::UDI_CONTROLLER => self::UDI_CONTROLLER,
        self::UDI_ACTION     => self::UDI_ACTION,
    );
    ...
    function reinit($full_init = false)
    {
            ...
            && ($url_mode == self::URL_MODE_PATHINFO || $url_mode == self::URL_MODE_REWRITE))
        {
            ...
            $this->_router->import(Q::ini('routes'), Q::ini('app_config/APPID') . '-' . 'global_default_routes');
		...
		{
			self::$_udi_param = array_merge(self::$_udi_param, Q::ini('app_config/UDI_PARAM', array()));
		}

        $udi = array();
        $udi[self::UDI_CONTROLLER] = (isset($keys[self::$_udi_param[self::UDI_CONTROLLER]]))
                                     ? $_GET[$keys[self::$_udi_param[self::UDI_CONTROLLER]]] : null;
        $udi[self::UDI_ACTION]     = (isset($keys[self::$_udi_param[self::UDI_ACTION]]))
                                     ? $_GET[$keys[self::$_udi_param[self::UDI_ACTION]]] : null;
        $udi[self::UDI_MODULE]     = (isset($keys[self::$_udi_param[self::UDI_MODULE]]))
                                     ? $_GET[$keys[self::$_udi_param[self::UDI_MODULE]]] : null;
        $udi[self::UDI_NAMESPACE]  = (isset($keys[self::$_udi_param[self::UDI_NAMESPACE]]))
                                     ? $_GET[$keys[self::$_udi_param[self::UDI_NAMESPACE]]] : null;
    ...
    function url($udi, $params = null, $route_name = null, array $opts = null)
    {
        ...
        else
        {
			...
			foreach($udi as $key => $value)
			{
				...
				if (self::$_udi_defaults[$key] == $value)
				{
					unset($udi[$key]);
					continue;
				}
                
				if(self::$_udi_param[$key] != $key)
				{
					$udi[self::$_udi_param[$key]] = $value;
					unset($udi[$key], $params[self::$_udi_param[$key]]);
				}
				else
				{
					unset($params[$key]);
    ...
    function normalizeUDI($udi, $return_array = true)
    {
        ...
        */
        //简化代码
        return $return_array ? $udi : $this->UDIString($udi);
    }
