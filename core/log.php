...
class QLog
{
    ...
    static function log($msg, $type = self::DEBUG)
    {
        ...
        if (is_null($instance))
        {
            ...
            //avoid double copied in memory
            //Q::singeleton也是在内存中存放一个副本
            $instance = new QLog();
        }
        /* @var $instance QLog */
    ...
    {
        if (!(bool) Q::ini('log_enabled')) return;
    ...
    function flush()
    {
        ...
        foreach ($this->_log as $offset => $item)
        {
            ...
            $string .= date(Q::ini('log_time_format', 'c'), $time) . " {$type}: {$msg}\n";
        }
        ...
        if ($string)
        {
            ...
            */
            //据说在大数据量并发写入时有BUG,待确认
            file_put_contents($this->_filename, $string, FILE_APPEND | LOCK_EX);
        }
	...
	function getFileName()
	{
        ...
        if($dotPos === false)
		{
			$filename = $filename . date('_Y-m-d');
		}
		else
		{
			$filename = substr($filename, 0, $dotPos) . date('_Y-m-d') . substr($filename, $dotPos);
		}
		return rtrim($dir, '/\\') . DS . $filename;
	}
}
