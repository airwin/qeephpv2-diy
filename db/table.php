...
class QDB_Table
{
    ...
    function __construct(array $config = null)
    {
        ...
        if (! empty($config['slave_conn']))
        {
            $this->setConn($config['slave_conn']);
        }
        if (! empty($config['master_conn']))
        {
            $this->setConn($config['master_conn'], 'master');
        }
    }
    ...
    function select()
    {
        ...
        $select = new QDB_Select($this->_slave_conn);
        $select->from($this);
        $args = func_get_args();
        if (! empty($args))
        {
     ...
     * @return array|null
     */
    function insert(array $row, $return_pk_values = false, array $_on_duplicate_key_update = array())
    {
        if (!$this->_inited)
        {
            $this->init();
        ...
        $this->_master_conn->insert($this->getFullTableName(), $row, self::$_fields[$this->_cache_id], $_on_duplicate_key_update);
        ...
        if ($return_pk_values)
        {
            ...
            if ($use_auto_incr)
            {
                $insert_id[$pk] = $this->_master_conn->insertID();
            }
            return $insert_id;
        }
        else
    ...
    function update($row, $where = null)
    {
        ...
        $this->_master_conn->update($this->getFullTableName(), $row, $where, self::$_fields[$this->_cache_id]);
    }
    ...
    function delete($where)
    {
        ...
        $this->_master_conn->delete($this->getFullTableName(), $where);
    }
    ...
    function nextID($field_name = null)
    {
        ...
        return $this->_slave_conn->nextID($this->getFullTableName(), $field_name);
    }
    ...
    function getConn($which = 'slave')
    {
        ...
        if($which == 'slave')
        {
            return $this->_slave_conn;
        }
        else
        {
            return $this->_master_conn;   
        }
    }
     ...
     */
    function setConn(QDB_Adapter_Abstract $conn, $which = 'slave')
    {
        if ($which == 'slave')
        {
            $this->_slave_conn = $conn;
            $conn = & $this->_slave_conn;
        }
        else
        {
            $this->_master_conn = $conn;
            $conn = & $this->_master_conn;
        }
		if (!$conn->isConnected())
        {
            $conn->connect();
    ...
    {
        if (is_null($this->_slave_conn))
        {
            $this->setConn(QDB::getConn());
        }
        if (is_null($this->_master_conn))
        {
            $this->setConn(QDB::getConn('master'), 'master');
        }
    }
    ...
    protected function _setupMeta()
    {
        ...
        $this->_cache_id = $this->_slave_conn->getID() . '-' . $table_name;
        ...
        // 从数据库获得 meta
        $this->_slave_conn->ping();
        $meta = $this->_slave_conn->metaColumns($table_name);
        $fields = array();
