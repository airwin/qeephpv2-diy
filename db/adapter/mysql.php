...
class QDB_Adapter_Mysql extends QDB_Adapter_Abstract
{
    ...
    {
        if (!$force_new && is_resource($this->_conn)) { return; }
    ...
    function ping($autoConnect = true)
    {
        $status = mysql_ping($this->_conn);
    ...
    function nextID_EX($table_name, $step = 1)
    {
        $seq_table_name = '`qee_sequence`';
        $table_name = $this->qstr($table_name);

        $next_sql = sprintf("INSERT INTO %s (`name`, `id`) VALUES(%s, last_insert_id(%d)) ON DUPLICATE KEY UPDATE `id` = LAST_INSERT_ID(`id` + %d)", $seq_table_name, $table_name, $step - 1, $step);
        ...
        {
            // 产生序列值失败，创建序列表
            $this->execute(sprintf("CREATE TABLE %s (`name` varchar(50) NOT NULL, `id` bigint(20) unsigned NOT NULL DEFAULT '0', PRIMARY KEY (`name`)) ENGINE=InnoDB", $seq_table_name));
        }
        ...
        // 获得新的序列值
        $this->_insert_id = $this->insertID() + 1;
    ...
    function metaColumns($table_name)
    {
        ...
        while (($row = $rs->fetchRow()))
        {
            ...
            elseif (preg_match('/^(.+)\((\d+)\)\s*(unsigned)?/', $type, $query_arr))
            {
                $field['type'] = $query_arr[1];
                $field['length'] = is_numeric($query_arr[2]) ? $query_arr[2] : - 1;
                if($query_arr[1] == 'int' && $query_arr[3] == 'unsigned')
                {
                    $field['type'] = 'float';
                }
            }
    ...
    protected function _fakebind($sql, array $inputarr)
    {
        //使用qinto完全可以替代_fakebind方法..
        return $this->qinto($sql, $inputarr);
