...
abstract class QDB_Adapter_Abstract
{
     ...
     * @param array $restricted_fields 限定只使用哪些字段
     * @param array $_on_duplicate_key_update on-duplicate-key-update的数据
     */
    function insert($table_name, array $row, array $restricted_fields = null, array $_on_duplicate_key_update = array())
    {
        ...
        if ($this->_bind_enabled)
        {
            ...
            if($_on_duplicate_key_update)
            {
                $pairs = array();
                foreach ($_on_duplicate_key_update as $field_name => $field_new_value)
                {
                    $pairs[] = $this->identifier($field_name) . ' = ' . (is_array($field_new_value) ? ($this->identifier($field_name) . ' ' . implode(' ', $field_new_value)) : $field_new_value);
                }
                if($pairs)
                {
    ...
    function update($table_name, $row, array $where = null, array $restricted_fields = null)
    {
        ...
        if ($this->_bind_enabled)
        {
            ...
            foreach ($holders as $key => $h)
            {
                list ($holder, $field_name, $df, $dv) = $h;
                if($df && $dv)
                {
                    $pairs[] = $field_name . ' = ' . $field_name . " {$df} " . $holder;
                }
                else
                {
            ...
            foreach ($values as $key => $holder)
            {
                ...
                {
                    $stmt->bindParam($holder, $row[$key][1]);
                }
                else
                {
                    $stmt->bindParam($holder, $row[$key]);
        ...
        else
        {
            ...
            foreach ($holders as $key => $h)
            {
                ...
                elseif(is_array($row[$key]))
                {
                    $pair .= "{$field_name} {$row[$key][0]} {$row[$key][1]}";
                }
                else
                {
    ...
    function getPlaceholder(array $inputarr, array $restricted_fields = null, $param_style = null)
    {
        ...
        foreach (array_keys($inputarr) as $offset => $key)
        {
            ...
            if(is_array($inputarr[$key]))
            {
                $holders[$key] = array_merge($holders[$key], $inputarr[$key]);
            }
        }
