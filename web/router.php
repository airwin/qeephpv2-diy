...
class QRouter
{
    ...
    function import(array $rules, $cache_id = null)
    {
        if ($cache_id)
        {
            ...
            $routes = Q::cache($cache_id, Q::ini('app_config/CONFIG_CACHE_SETTINGS/' . $backend), $backend);
        }
        ...
        if (!$cache_id || !$routes)
        {
            ...
            if ($cache_id)
            {
                Q::writeCache($cache_id, $routes, Q::ini('app_config/CONFIG_CACHE_SETTINGS/' . $backend), $backend);
            }
        }
