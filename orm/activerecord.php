                                                    ...
                                                    ArrayAccess
{
     ...
     * 指示哪些字段是增/减操作
     *
     * @author mabiao@staff.sina.com.cn
     * @var array
     */
    private $_xcrement_props = array();
     ...
     * @param array $_on_duplicate_key_update insert操作发生duplicate时的选项
     *
     * @return QDB_ActiveRecord_Abstract 连贯接口
     */
    function save($recursion = 99, $save_method = 'save', array $_on_duplicate_key_update = array())
    {
        if(!is_numeric($recursion) && in_array($recursion, array('create', 'update', 'replace', 'save'), true))
        {
            $tmp = intval($save_method);
            $save_method = $recursion;
            $recursion = $tmp;
        }
        ...
        try
        {
            ...
            case 'create':
                ...
                $this->_create($recursion, $_on_duplicate_key_update);
                break;
            case 'update':
     ...
     *
     * @param string $prop_name 要改变的属性名
     * @param float $prop_value 要增减的数值
     */
    function xCrease($prop_name, $prop_value = 1, $save = false)
    {
        $meta = self::$_meta[$this->_class_name];
        /* @var $meta QDB_ActiveRecord_Meta */
        ...
        {
            throw new QDB_ActiveRecord_SettingPropTypeMismatchException($this->_class_name, $prop_name);
        }
        
        if($prop_value != 0)
        {
            $this->_xcrement_props[$prop_name] += $prop_value;
            $this->_props[$prop_name] = $this->_props[$prop_name] + $prop_value;
            $this->_changed_props[$prop_name] = $prop_name;
        }
        if($save)
        {
            $this->save(intval($save), 'update');
        }
    }
	...
	/**
     * 魔法方法，用于调用行为插件为对象添加的方法 [php5.3+可用]
     *
     * @param string $method
     * @param array $args
     *
    ...
    {
        $_class_name = get_called_class();
		if (! isset(self::$_meta[$_class_name]))
        {
            self::$_meta[$_class_name] = QDB_ActiveRecord_Meta::instance($_class_name);
        }
        return call_user_func_array(array(self::$_meta[$_class_name], $method_name), $args);
    }
    ...
    /**
     * 触发事件，将原本只允许模型内部的触发现在允许模型外部触发，
     * 这在流程上算是一个很大的改变，模型的事件将被允许在controller中触发
     * 由于可能会和模型内部的事件冲突，这里强制$event为整形，区别模型中的字符型
     *
     * @param int $event
     */
    public function event($event)
    {
        ...
        Helper_Event::doEvent($event, $this);
    }
	...
	{
		$event_arr = func_num_args() > 1 ? func_get_args() : (!is_array($events) ? explode(',', $events) : $events);
     ...
     * @param int $recursion
     */
    protected function _create($recursion = 99, array $_on_duplicate_key_update = array())
    {
        $meta = self::$_meta[$this->_class_name];
        /* @var $meta QDB_ActiveRecord_Meta */
        ...
        $pairs = array();
        foreach ($_on_duplicate_key_update as $prop_name => $value)
        {
            $pairs[$meta->props2fields[$prop_name]] = $value;
        }
        
        // 将名值对保存到数据库
        $pk = $meta->table->insert($save_data, true, $pairs);
    ...
    protected function _update($recursion = 99)
    {
        ...
        if (!empty($this->_changed_props))
        {
	        ...
	        foreach ($this->_props as $prop_name => $value)
	        {
	            ...
	            if (isset($this->_changed_props[$prop_name]))
	            {
	                if(isset($this->_xcrement_props[$prop_name]) && $this->_xcrement_props[$prop_name] != 0)
	                {
                        $save_data[$meta->props2fields[$prop_name]] = array($this->_xcrement_props[$prop_name] > 0 ? '+' : '-', abs($this->_xcrement_props[$prop_name]));
	                }
	                else
	                {
	                    $save_data[$meta->props2fields[$prop_name]] = $value;
